...
FONT_SIZE = 11
MENU_SIZE = 40 # This is the size which is taken from the menus of the editor
...
class AutocompleteText(Text):
    ...
    def position_menu(self):
        ...
        menu_x = self.root.winfo_x() +\
            int(FONT_SIZE * (len(self.get(row + '.0', INSERT)) + 1) / 2)
        menu_y = self.root.winfo_y() + MENU_SIZE + FONT_SIZE * int(row)
        # print("Menu X: {}, Menu Y:{}".format(menu_x, menu_y))
        print("Expected Coordinates:\nX: {}, Y: {}".format(self.root.winfo_pointerx(),
